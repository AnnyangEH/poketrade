// DEPRECATED 2026-07-16 - 예약 시스템 복구로 인해 주석처리 (클릭형 이미지 인벤토리 선택 UI)
// /* ════════════════════════════════════════
...
// 
// function spriteUrl(dexId, formId) {
//   // 기본 도감번호 아트워크는 일반/이로치에만 정확히 맞음 — 갈라르/오리진/화신폼 등
//   // 별도 폼은 도감번호만으로 못 가져와서(별도 PokeAPI 조회 필요) 지금은 제외하고 삼각형 유지
//   if (formId === 'normal') return `${SPRITE_BASE}/${dexId}.png`;
//   if (formId === 'shiny')  return `${SPRITE_BASE}/shiny/${dexId}.png`;
//   return null;
// }
// 
...
//     iconUrl: spriteUrl(p.dexId, f.formId),
...
//   cell.className = 'relative aspect-square cursor-pointer flex items-center justify-center overflow-hidden' +
...
//     img.loading = 'lazy';
//     img.onerror = () => { img.remove(); cell.appendChild(buildTriangle(e)); };
//     cell.appendChild(img);
//   } else {
//     cell.appendChild(buildTriangle(e));
//   }
// 
//   cell.onclick = () => {
